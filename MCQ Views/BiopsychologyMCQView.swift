...
import SwiftUI
struct BiopsychologyMCQView: View {
        ...
        NavigationView {
            ScrollView {
                ...
                VStack(spacing: 1.0) {
                    ...
                    Group {
                        VStack {
                            Text(BiopsychologyMCQ[questionIndex].mcqquestion)
                        ...
                        VStack {
                            Toggle(BiopsychologyMCQ[questionIndex].mcqoptions[0], isOn: OptionOne)
                                .toggleStyle(.button)
                                .padding(EdgeInsets(top: 0, leading: 20, bottom: 0, trailing: 20))
                                .tint(Color(red: 0.8, green: 0.8, blue: 0.8))
                                .foregroundColor(Color("Black-White"))
                            Toggle(BiopsychologyMCQ[questionIndex].mcqoptions[1], isOn: OptionTwo)
                                .toggleStyle(.button)
                                .padding(EdgeInsets(top: 0, leading: 20, bottom: 0, trailing: 20))
                                .tint(Color(red: 0.8, green: 0.8, blue: 0.8))
                                .foregroundColor(Color("Black-White"))
                            Toggle(BiopsychologyMCQ[questionIndex].mcqoptions[2], isOn: OptionThree)
                                .toggleStyle(.button)
                                .padding(EdgeInsets(top: 0, leading: 20, bottom: 0, trailing: 20))
                                .tint(Color(red: 0.8, green: 0.8, blue: 0.8))
                                .foregroundColor(Color("Black-White"))
                    ...
                    HStack(spacing: 15) {
                        ForEach(0..<button.count, id: \.self) {button in
                            Button {
                                ...
                                if BiopsychologyMCQ.count > questionIndex + 1 {
                                    questionIndex += 1
                                }
                            } label: {
                                ZStack {
                                    ...
                                    if questionIndex == 4 {
                                        NavigationLink("Confirm Answer", destination: BiopsychologySAView())
                                        .foregroundColor(.white)}
                                }}
            ...
            .navigationTitle("Biopsychology")
            .navigationBarTitleDisplayMode(.inline)
        }
    }
}

struct BiopsychologyView_Previews: PreviewProvider {
    static var previews: some View {
        BiopsychologyMCQView()
